...
class Piece:
    ...
    # reachable_states method for each piece type
...
class Board:
            ...
            # start of friendly pieces
                ...
                # get goal position
                    ...
                    for goal in goalPos:
                        ...
                        self.goalStates.append((goalCol, goalRow))
    ...
    def expandNode(self, currNode):
        currPos, currPath, currPathCost = currNode.currPos, currNode.path, currNode.pathCost
        ...
        for pos in possiblePos:
            ...
            if self.isEmpty(pos) or self.isGoal(pos):
                ...
                newPathCost = currPathCost + int((self.boardDict[pos]).stepCost)
                newEuclidDist = self.getMinEuclideanDistance(pos)
                nextStates.append(Node(pos, newPath, newPathCost, newEuclidDist))
        return nextStates
    
    def getMinEuclideanDistance(self, pos):
        ans = self.findEuclideanDistance(0, 0, self.col, self.row)
        for goal in self.goalStates:
            currDist = self.findEuclideanDistance(ord(pos[0]), pos[1], ord(goal[0]), goal[1])
            if currDist < ans:
                ans = currDist
        return ans

    def findEuclideanDistance(self, x1, y1, x2, y2):
...
class Node:
    ...
    def __init__(self, currPos, path, pathCost, euclidDistance):
        self.currPos = currPos
        self.path = path
        self.pathCost = pathCost
        self.euclidDistance = euclidDistance
    
    # overload less than or equal to operator
    def __lt__(self, other):
        return (self.pathCost + self.euclidDistance) < (other.pathCost + other.euclidDistance)
...
def search():
    ...
    initEuclidDist = board.getMinEuclideanDistance(startPos)
    # PQ for UCS
    startNode = Node(startPos, [], 0, initEuclidDist)
    frontier = [startNode]
    heapq.heapify(frontier)

    # reached hash table
    reached = {}
    reached[startPos] = startNode
    ...
    while frontier:
        ...
        nodesExplored += 1
        if board.isGoal(currNode.currPos):
            return currNode.path, nodesExplored, currNode.pathCost
        # go through possible nextstates
        for child in board.expandNode(currNode):
            #print("expanded")
            currPos = child.currPos 
            if currPos not in reached or child.pathCost < (reached[currPos]).pathCost:
                reached[currPos] = child
